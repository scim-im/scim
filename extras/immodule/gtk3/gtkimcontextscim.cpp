...
// and the candidates would simply never appear. Opened the first time such a
// client is focused, so an ordinary application never pays for the extra
// display connection.
...
#ifdef SCIM_HAS_CANDIDATES_SINK
        // Whether a popover can be anchored is a property of the client, so the
                                         ...
                                         GdkRectangle *area)
{
    ...
    if (context_scim && context_scim->impl && context_scim->impl->client_window && context_scim == _focused_ic) {
        ...
        if (context_scim->impl->cursor_x != x + area->x + area->width ||
            ...
            context_scim->impl->cursor_y = y + area->y + area->height + 8;
            // The popover is told to point at the caret rectangle itself, not
            // at a nudged-away point: it places itself clear of that rectangle,
            // so an offset here only pushes the candidates further from the
            // text. Keep it in client-window coordinates, which is the space
            // GtkPopover reads pointing_to in.
...
#ifdef SCIM_HAS_CANDIDATES_SINK
            if (_sink)
                _sink->update_spot_location (context_scim->impl->cursor_x,
                                             context_scim->impl->cursor_y);
#endif
            _panel_client.prepare (context_scim->id);
            _panel_client.send ();
...
// The widget a context's client window belongs to, or null when nothing owns
// that window. Chromium creates its GdkWindow itself and attaches no widget, so
// this is the test for "the popover has nothing to hang off".
static GtkWidget *
ic_widget (GtkIMContextSCIM *ic)
{
...
public:
    ...
    // The popover handles its own pointer events and calls the same handlers
...
/*                                                                     */
/* Shared by the popover's own pointer handling and by kimpanel, which  */
...
#endif
#ifdef SCIM_HAS_CANDIDATES_X11
    // A popover has to hang off a widget. When the focused client has none,
    // draw a window of our own instead -- otherwise there is nowhere to put the
    // candidates and none appear at all.
    if (!want && _focused_ic && !ic_widget (_focused_ic) && candidates_x11_ensure ())
